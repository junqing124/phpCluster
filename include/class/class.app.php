...
{   
    ...
    public static function get_db_by_info( $db_type, $db_host, $db_name, $db_pass, $db_table, $db_ut )
    {
        return new cls_db( $db_type, $db_host, $db_name, $db_pass, $db_table, $db_ut );
    }
    ...
    {
        return cls_redis::get_redis_instance();
    }
     ...
     * @return Redis 一个redis实例
     */
    public static function get_redis_slave( )
    {
    ...
    public static function log_ex( $ack, $message = '' )
    {
        $file_name = $_SERVER['SCRIPT_FILENAME'];
        $fn_info = pathinfo( $file_name );
        $script_name = $fn_info['basename'];
        $cls_log = new cls_log();
        $cls_log->set_collection( 'log_system_log' );
        $info = array( 'sl_add_day'=> date('Y-m-d'), 'sl_script_name'=> $script_name, 'sl_ack'=> $ack, 'sl_message'=> $message, );

        $cls_log->add_log( $info );
    }
    ...
    public static function get_db_slave_info( $rand_slave_id = 1, $slave_id = 0 )
    {
    ...
    {
        $slave_info = self::get_db_slave_info( $rand_slave_id, $slave_id );
        if( $slave_info['db_host'] )
        {
    ...
    //以一个key来存值 用redis来存 默认是22天
    public static function set_key_cache( $key, $value, $exp_time = 1900800 )
    {
        if( class_exists( 'Redis' ) )
        {
            $redis = cls_app:: get_redis();
            $redis->set_value( $key, $value, $exp_time );
            return true;
        }else
        {
            return false;
    ...
    //获取key来存值 用redis来存 默认用从的
    public static function get_key_cache( $key/*, $is_slave = 1*/ )
    {
        if( class_exists( 'Redis' ) )
        {
            /*if( $is_slave )
            {
                //echo 'a';
                //echo '<hr>';
                $redis = cls_app:: get_redis_slave();
            }else
            {
                $redis = cls_app:: get_redis();
            }*/
            $redis = cls_app:: get_redis();
            $info_str = $redis->get_value( $key );
            return $info_str;
        }else
        {
            return false;
    ...
    //删除redis缓存key
    public static function del_key_cache( $key )
    {
        if( class_exists( 'Redis' ) )
        {
            $redis = cls_app:: get_redis();
