...
class cls_data
{
	...
	public function select_all($canshu = array())
	{
	 ...
	 */
	function insert( $info, $return_sql_only = 0, $return_insert_id = 1, $is_ignore = 0 )
	{
        $key_list = implode( '`,`', array_keys($info) );
        $key_list = '`' . $key_list . '`';
        $value_list = implode( "','", array_values($info) );
        $value_list = "'" . $value_list . "'";
		$ignore_str = $is_ignore ? 'ignore' : '';
        $sql = 'insert '. $ignore_str .' into ' . $this->table . "($key_list) values($value_list)";
     ...
     * @return int 返回值为文档的ID,失败返回0
     */
    function insert_ex( $info, $return_insert_id = 0, $return_sql_only = 0, $is_ignore = 0 )
    {
        return $this->insert( $info, $return_sql_only, $return_insert_id, $is_ignore );
    }
	 ...
	 * @return int 返回值为文档的ID,失败返回0
	 */
	function insert_bulk( $info, $return_sql_only = false, $is_ignore = 0 )
	{
	...
	//批量修改
	function update_all($info = array(), $where= '', $return_sql_only = false)
	{
		$sql = $this->update( $info, $where, true );
